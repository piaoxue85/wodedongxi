...
class LinearRegression(nn.Module):
    ...
    def forward(self, X ,code_return  ):
        out  = Variable(torch.from_numpy(np.array([-99999],dtype=np.float32)))
        
        data = Variable(torch.from_numpy(np.array(X["code"].values,dtype=np.float32)))
        X = X.drop('code', 1)
