...
def my_Pro_bar(ts_code ,pro_api , start_date="", end_date="",adj="adj",asset=""):
    ...
    return df1.append([df2,df3,df4,df5,df])
...
#                 cons = ts.get_apis()          
            ...
            data  = ts.pro_bar(code, conn=cons, adj='qfq', start_date=shi_jian.replace("-", ""), end_date=shi_jian.replace("-", ""),retry_count = 2)
...
def importStockDataDailyZS(codes= [],names=[],asset='E',adj='qfq'):
    global cons
    db=cx_Oracle.connect('c##stock','didierg160','myoracle')  #创建连接  
    cr=db.cursor()
