...
class Qnetwork():
    def __init__(self,h_size):
        #double DQN 中的主DQN
        self.scalarInput = tf.placeholder(shape= [None,4096*1] , dtype = tf.float32)
        self.imageIn     = tf.reshape(self.scalarInput, shape = [-1 , 64 *1 ,64 *1 , 1] )
...
def processState(states):
    #print(states.shape)
    return np.reshape(states,[4096*1])
...
with tf.Session() as sess:
    ...
    for i in range(num_episodes):
        ...
        while j < max_epLength: #If the agent takes longer than 200 moves to reach either of the blocks, end the trial.
            ...
            randtmp =np.random.rand(1)
            if randtmp < e or total_steps < pre_train_steps:
        ...
        if i % 100 == 0:
            saver.save(sess,path+'/model-'+code+'_'+str(i)+'.cptk')
            print("Saved Model")
