...
flags.DEFINE_string("train_tfrecords_file",
                    "./data/cancer/StockTrain.csv.tfrecords",
                    "The glob pattern of train TFRecords files")
flags.DEFINE_string("validate_tfrecords_file",
                    "./data/cancer/StockTest.csv.tfrecords",
                    "The glob pattern of validate TFRecords files")
...
flags.DEFINE_string("inference_result_file", 
                    "./inference_result.txt",
                    "The result file from inference")
flags.DEFINE_string("optimizer", "adam", "The optimizer to train sgd adadelta adagrad adam ftrl rmsprop")
flags.DEFINE_string("model", "resnet", "Support dnn, lr, wide_and_deep,cnn,resnet,bidirectional_rnn")
...
flags.DEFINE_integer("label_size", 3, "Number of label size")
...
flags.DEFINE_integer("epoch_number", 4000, "Number of epochs to train")
...
flags.DEFINE_integer("batch_size", 128, "The batch size of training")
...
flags.DEFINE_string("output_path", "D:/deep_recommend_system-master/tensorboard/",
                    "The path of tensorboard event files")
flags.DEFINE_boolean("enable_bn", False , "Enable batch normalization or not")
flags.DEFINE_float("bn_epsilon", 0.0001, "The epsilon of batch normalization")
...
#     inputs = tf.reshape(inputs, [FLAGS.batch_size,120,7,1])
  ...
  def resnet_inference(inputs, is_train=True):
    ...
    with slim.arg_scope(resnet_arg_scope(is_training=(MODE == "train"))) :     
        net,enpoints = resnet_v2.resnet_v2_50(x, num_classes=FLAGS.label_size)
  ...
  if MODEL == "bidirectional_rnn": 
    train_correct_prediction = tf.equal(tf.argmax(train_accuracy_logits,1), tf.argmax(batch_labels,1))
  else : 
  ...
  if MODEL == "bidirectional_rnn": 
    validate_correct_prediction   = tf.equal(tf.argmax(validate_accuracy_logits,1), tf.argmax(validate_batch_labels,1))
  else : 
