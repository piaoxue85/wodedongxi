...
# flags.DEFINE_string("train_tfrecords_file",
#                     "./data/lung/StockTrain.csv.tfrecords",
#                     "The glob pattern of train TFRecords files")
# flags.DEFINE_string("validate_tfrecords_file",
#                     "./data/cancer/StockTest.csv.tfrecords",
#                     "The glob pattern of validate TFRecords files")
# flags.DEFINE_string("inference_test_file", 
#                     "./data/cancer/StockInference.csv",
#                     "The test file for inference")
# flags.DEFINE_string("inference_result_file", 
#                     "./inference_result.txt",
#                     "The result file from inference")
flags.DEFINE_string("train_tfrecords_file",
                    "./data/cancer/MNIST_train.tfrecords",
                    "The glob pattern of train TFRecords files")
flags.DEFINE_string("validate_tfrecords_file",
                    "./data/cancer/MNIST_test.tfrecords",
                    "The glob pattern of validate TFRecords files")
flags.DEFINE_string("inference_test_file", 
                    "./data/cancer/StockInference.txt",
                    "The test file for inference")
flags.DEFINE_string("inference_result_file", 
                    "./inference_result.txt",
                    "The result file from inference")
flags.DEFINE_string("optimizer", "adam", "The optimizer to train sgd adadelta adagrad adam ftrl rmsprop Momentum")
flags.DEFINE_string("model", "cnn", "Support dnn, lr, wide_and_deep,cnn,resnet")
flags.DEFINE_string("mode", "train", "Support train, export, inference")
# flags.DEFINE_integer("feature_size", 3840, "Number of feature size")
flags.DEFINE_integer("feature_size", 4096, "Number of feature size")
# flags.DEFINE_integer("feature_size", 2048, "Number of feature size")
flags.DEFINE_integer("label_size", 11, "Number of label size")
...
flags.DEFINE_integer("epoch_number", 10000 , "Number of epochs to train")
# flags.DEFINE_integer("batch_size", 15269 , "The batch size of training")
# flags.DEFINE_integer("validate_batch_size", 15269 ,
#                     "The batch size of validation")
flags.DEFINE_integer("batch_size", 50 , "The batch size of training")
flags.DEFINE_integer("validate_batch_size", 50 ,
                     "The batch size of validation")
...
flags.DEFINE_string("checkpoint_path", "./cnn_checkpoint/",
                    "The path of checkpoint")
# flags.DEFINE_string("output_path", "./tensorboard/",
#                     "The path of tensorboard event files")
flags.DEFINE_string("output_path", "D:\\deep_recommend_system-master\\tensorboard\\",
                    "The path of tensorboard event files")
flags.DEFINE_boolean("enable_bn", False , "Enable batch normalization or not")
flags.DEFINE_float("bn_epsilon", 0.01, "The epsilon of batch normalization")
...
flags.DEFINE_string("model_path", "./cnn_model/", "The path of the model")
...
#   fac = np.load('D:/share/fac16.npy').astype(np.float32)
#   ret = np.load('D:/share/ret16.npy').astype(np.float32) 
#   print(fac.shape) ;
#   print(ret.shape) ;   
...
#     inputs = tf.reshape(inputs, [FLAGS.batch_size,120,7,1])
    inputs = tf.reshape(inputs, [FLAGS.batch_size,28,28,1])
    ...
    with tf.variable_scope("output"):
      weights = tf.get_variable("weights", [7 * 7 * 8, LABEL_SIZE],
                                initializer=tf.random_normal_initializer())
