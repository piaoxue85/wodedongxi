...
def select_strategy():
    ...
    g.cs_enabled,g.cs_memo,g.cs_class_name,g.cs_param = list(range(4))
...
def clear_position(sender,context):
    # 用于发邮件
    g.position = list(context.portfolio.positions.keys())
    g.stocks_to_buy = []
...
def get_return_values(date_list, factors, values_return_dict_history):
    try:
        date_history = list(values_return_dict_history.keys())
    ...
    for date in date_temp_list:
        ...
        # 如果已经存在，则不计算
        if enddate in list(result.keys()):
...
def caculate_score(scores_return_panel, date_score, effect_test_df_T, effective_factors):
    ...
    for column in timesOfReturn:
        ...
        score_df.reset_index(list(range(1,len(score_df) + 1)), inplace = True)
